        ...
        <Layout>
            <div className="mx-auto max-w-5xl p-6 md:px-12 lg:px-24 text-left">
                <section className="mb-12">
                    <h2 className="text-2xl font-semibold mb-4">C言語コード全文</h2>
...
int play_player(Card deck[], int* deck_pos, Card hand[]) {
    ...
    while (1) {
        ...
        if (total == 21) { // 21なら自動的にスタンド
            printf("21になったので自動的にスタンドします。\n");
            break;
        }
        int choice;
        printf("ヒット(1) か スタンド(0)？: ");
        if (scanf("%d", &choice) != 1) { // 入力エラー
            printf("入力エラーです。スタンドします。\n");
            while (getchar() != '\n'); // バッファクリア
            break;
        }
        if (choice == 1) { // ヒット
...
int main() {
    ...
    while (money > 0) {
        ...
        if (money <= 0) { // 所持金が0以下で終了
            printf("所持金がなくなりました。ゲーム終了です。\n");
            break;
        }
        printf("\n次のゲームを始めます...\n\n");
    }
    return 0; // 正常終了           `} />
                            <div className="mt-4 text-zinc-200 text-sm leading-7"></div>
                        </TabsContent>
                    </Tabs>
                </section>
